...
async function fetchSongs() {
  try {
    // 음악 목록 가져오기
    const responseSongs = await fetch('http://219.241.14.3:41309/api/music-server/songs');
    const dataSongs = await responseSongs.json();
    songs = dataSongs.songs;

    // 아티스트 목록 가져오기
    const responseArtists = await fetch('http://219.241.14.3:41309/api/music-server/artist');
